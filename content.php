...
case 'category':
	...
	break;
case 'trending':
    $headtitle='トレンド'.'-'.SITE_NAME;
	break;
case 'DMCA':
    $headtitle='DMCA'.'-'.SITE_NAME;
	break;
case 'video':
    $headtitle='ビデオダウンロードツール'.'-'.SITE_NAME;
	break;
case 'api':
         ...
         <div class="col-md-9 relatedlist">
            ...
            switch ($cont) {
		        ...
		        echo '<div id="videocont"></div>
            		...
            		echo '<option '. $relevance .' data-url="./ajax/ajax.php?type=category&sortid='.$sortid.'&ptk='.$ptk.'">人気動画</option>';
            		echo '<option ' . $date1 .' data-url="./ajax/ajax.php?type=category&sortid='.$sortid.'&order=date&ptk='.$ptk.'">最新の動画</option>';
            		echo '<option ' . $viewCount .' data-url="./ajax/ajax.php?type=category&sortid='.$sortid.'&order=viewCount&ptk='.$ptk.'">再生回数の多い動画</option>';
            		echo '</select></div></div>';
