...
$(document).ready(function(){
 ...
 <div class="col-md-4 col-sm-12 col-xs-12 col-lg-4 equipodiv order-md-1 order-lg-1 order-xs-2 order-sm-2">
  <div class="related">
      <p class="font-weight-bold h6 pb-1">チャンネルの情報</p>
       <span class="d-block py-3 fa fa-calendar"> 登録日時<?php echo date("Y-m-d", strtotime($channel['items'][0]['snippet']['publishedAt']));?></span>  
       <span class="d-block py-3 fa fa-play"> <?php echo $channel['items'][0]['statistics']['viewCount']?>総視聴回数</span> 
       <span class="d-block py-3 fa fa-user-circle-o"> <?php echo $channel['items'][0]['statistics']['subscriberCount']?>登録者数</span>
       <span class="d-block py-3 fa fa-globe"> 国<?php if (array_key_exists('country',$channel['items'][0]['snippet'])) {echo get_country($channel['items'][0]['snippet']['country']);} else {echo '火星';}?></span> 

  </div>
  <div class="related text-nowrap">
      <p class="font-weight-bold h6 pb-1">関連チャンネル</p>
       <?php
 ...
 <div class="col-md-8 col-sm-12 col-xs-12 col-lg-8 related order-md-2 order-lg-2 order-xs-1 order-sm-1">
    <div class="font-weight-bold h6 pb-1">全ての動画（<?php echo $channel['items'][0]['statistics']['videoCount'] ?>個）</div> 
